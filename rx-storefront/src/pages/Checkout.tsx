 ...
 * On success we go to /offer?orderId=&checkoutToken= — intake is on thank-you.
 */
...
function CheckoutResume() {
  ...
  usePayment({
    onPaymentSuccess: (result) => {
      ...
      const checkoutToken = parseTokensFromUrl()?.checkoutToken ?? '';
      const query = new URLSearchParams({ orderId });
      if (checkoutToken) query.set('checkoutToken', checkoutToken);
      navigate(`/offer?${query.toString()}`, { replace: true });
    },
...
function CheckoutForm({
  ...
  const { loadPaymentSetup, tokenizeCard, processPayment, isProcessing } = usePayment({
    onPaymentSuccess: (result) => {
      const orderId = result.order?.id;
      if (orderId) {
        const query = new URLSearchParams({ orderId, checkoutToken });
        navigate(`/offer?${query.toString()}`, { replace: true });
      }
    },
  });
