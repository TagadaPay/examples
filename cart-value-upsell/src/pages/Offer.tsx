...
type OfferPageProps = {
  ...
  acceptPath: string;
  declinePath: string;
  /** Which orchestrator condition picked this offer — shown for the demo. */
  matched?: string;
  imageUrl?: string;
};
  ...
  return (
    <main className="page">
      ...
      <div className="card offer">
        ...
        {imageUrl ? <img className="offer-image" src={imageUrl} alt={title} /> : null}
        <h1>{title}</h1>
        <p className="price">{priceLabel}</p>
        {matched ? (
          <p className="hint">
            Routed by <code>{matched}</code>
          </p>
        ) : null}
