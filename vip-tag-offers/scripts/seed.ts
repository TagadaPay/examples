#!/usr/bin/env tsx
/**
 * Creates the catalog + offers for the VIP tag offers tutorial:
 * tee $29.99 for everyone, then one OTO per segment —
 * VIP (tag: vip) Members Hoodie $39, everyone else Welcome Cap $14.99.
 *
 * Usage:
 *   pnpm seed
...
async function main() {
  console.log('\n  Seeding VIP tag offers catalog\n');

  const { processor } = await tagada.processors.create({
    processor: {
      name: 'VIP tag offers sandbox',
      type: 'sandbox',
      enabled: true,
  ...
  console.log('  products   tee / hoodie (VIP) / cap (welcome)');

  const vipOffer = await createUpsellOffer({
    storeId: store.id,
    title: 'Members Hoodie — $39 (VIP)',
    product: hoodie,
  });
  const welcomeOffer = await createUpsellOffer({
    storeId: store.id,
    title: 'Welcome Cap — $14.99',
    product: cap,
  });
  console.log(`  offers     VIP ${vipOffer.id} / welcome ${welcomeOffer.id}`);
  ...
  const env = [
    ...
    `VITE_VIP_OFFER_ID=${vipOffer.id}`,
    `VITE_WELCOME_OFFER_ID=${welcomeOffer.id}`,
    '',
  ]
    .filter((line) => line !== '')
    .join('\n');
